...
tape('Server and validate function should work correctly', (t) => {
  ...
  server.init.inject({method: 'GET', url: '/admindotfilter', headers: {authorization: basicHeader('MattTheHacker', '<scriptInjection11!>')}}, (response) => {
  ...
  server.init.inject({method: 'GET', url: '/admindotfilter', headers: {authorization: basicHeader('2093084832fapdsahf', 'ur89ea0whpgu24')}}, (response) => {
	...
	server.init.inject({method: 'GET', url: '/admindotfilter', headers: {authorization: basicHeader('jack', 'fish')}}, (response) => {
    t.equal(response.statusCode, 200,'Server returns 401 Unauthorised (without password)');
  });
  server.init.inject({method: 'GET', url: '/admindotfilter', headers: {authorization: basicHeader('owen', 'the redis is the best')}}, (response) => {
    t.equal(response.statusCode, 200,'Server returns 401 Unauthorised (without password)');
  });
  server.init.inject({method: 'GET', url: '/admindotfilter', headers: {authorization: basicHeader('andrew', 'bosh')}}, (response) => {
    t.equal(response.statusCode, 200,'Server returns 401 Unauthorised (without password)');
  });
});
